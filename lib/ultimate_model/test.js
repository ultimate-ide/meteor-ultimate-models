var trans = Transaction.findOne({}, {limit: 1, sort: {date_created: -1}}),
	lastCreated = moment(trans.created_at);

gateway.transaction.search(function (search) {
  	search.createdAt().min(lastCreated);
}, function (err, response) {
	response.each(function (err, transaction) {
    	var trans = new Transaction;
    	trans.addTransaction(transaction.customFields, transaction);
    	trans.save();
  	});
});


Ultimate('Transaction').extends(UltimateModel, {
	relations: {
		agent: {
			relation: 'belongs_to',
			model: 'Agent',
	...
	addTransaction: function(trans, fullTrans) {
		var agent = Agents.findOne({code: trans.agentCode}),
			agentId = agent ? agent._id : 'incorrect-agent-code',
			customer = Customer.createIfNotExists(customerKey, customerName, agentId),
			celeb = Celebrity.createIfNotExists(customerKey, customerName, agentId, customer._id);

		customer.celebrity_id = celeb._id;

		this.agent_id = agentId;
		this.customer_id = customer._id;
		this.celeb_id = celeb._id;

		this.agent_name = agent.full_name;
		this.customer_name = customer.full_name;
		this.celebrity_name = celeb.full_name;

		this.agent_code = trans.agentCode;
		this.cost = trans.cost;

		this.assignOrderInfo(trans.orderInfo);
	},
	assignOrderInfo: function(info) {
...
Ultimate('Person').extends(UltimateModel, {
	subscriptions: {
		all: {
			selector: {}
		}
	},
	assignFullname: function(fullName) {
		this.full_name = fullName;
		this.first_name = _.first(fullName.split(' '));
		this.last_name = _.last(fullName.split(' '));
	}
}, {
	datatableSubscribe: function(relations) {
		return this.with.apply(this, arguments).handle().subscribe('all')
	}
});

Ultimate('Customer').extends(Person, {
	relations: {
		firstAgent: {
			relation: 'belongs_to',
			model: 'Agent',
	...
	createIfNotExists: function(customerKey, customerName, agentId) {
		var customer = Customers.findOne({customer_key: customerKey});
		if(customer) return customer;

		customer = new Customer;

		customer.customer_key = customerKey;
		customer.assignFullname(customerName);
...
Ultimate('Celebrity').extends(Person, {
	relations: {
		agent: {
			relation: 'belongs_to',
			model: 'Agent',
	...
	createIfNotExists: function(celebrityKey, celebrityName, agentId, customerId) {
		var celeb = Celebrities.findOne({celebrity_key: celebrityKey});
		if(celeb) return celeb;

		celeb = new Celebrity;

		celeb.celebrity_key = celebrityId;
		celeb.assignFullname(celebrityName);
		celeb.agent_id = agentId;

		celeb.customer_id = customerId;
...
Ultimate('Agent').extends(Person, {
	relations: {
		...
		totalEarnedDuringTrial: {
			...
			selector: function() {
				return {
					created_at: {
				        $gte: this.trial_start_date,
				        $lte: this.getTrialEndDate();
				    }
				}
			}
		},
		totalEarnedDuringProratedSecondMonth: {
			relation: 'aggregate',
			model: 'Transaction',
			foreign_key: 'agent_id',
			operator: 'sum',
			...
			selector: function() {
				return {
					created_at: {
				        $gt: this.getTrialEndDate();,
				        $lte: DateUtilities.getLastDateOfMonth()
				    }
				}
			}
		},
		totalEarnedDuringRegularMonth: {
			relation: 'aggregate',
			model: 'Transaction',
			foreign_key: 'agent_id',
			operator: 'sum',
			...
			selector: function() {
				return {
					created_at: {
				        $gte: DateUtilities.getFirstDateOfMonth(),
				        $lte: DateUtilities.getLastDateOfMonth()
				    }
				}
			}
	...
	setTrialStartDate: function(date) {
		this.trial_start_date = date;
	},
	getTrialStartDate: function() {
		return this.trial_start_date;
	},
	getTrialEndDate: function() {
		return moment(this.trial_start_date).add(30, 'days');
	}
});


Ultimate('DateUtilities').extends({}, {
...
Ultimate('Payment').extends(UltimateModel, {
	...
	onStartup: function() {
		this.setDailyInterval();
	},

	setDailyInterval: function() {
		this.setTimeout(this.createPayments, 10 * 60 * 1000); //charge some orders on every startup
		this.setInterval(this.createPayments, 24 * 60 * 60 * 1000);
	},
	createPayments: function() { 
		Agents.find().forEach(function(agent) {
			var total = Transactions.sum('cost', {agent_id: agent._id, created_at: {
			        $gte: agent.getCycleStart(),
			        $lt: agent.getCycleEnd()
			    }
			});

			Payments.upsert({
				start_date: agent.getCycleStart(),
				end_date: agent.getCycleEnd()
			}, {
				total: total,
				start_date: agent.getCycleStart(),
				end_date: agent.getCycleEnd(),
		...
		Celebrities.find().forEach(function(celeb) {
			var total = Transactions.sum('cost', {celebrity_id: celeb._id, created_at: {
			        $gte: celeb.getCycleStart(),
			        $lt: celeb.getCycleEnd()
			    }
			});

			Payments.upsert({
				start_date: celeb.getCycleStart(),
				end_date: celeb.getCycleEnd()
			}, {
				total: total,
				start_date: celeb.getCycleStart(),
				end_date: celeb.getCycleEnd(),
