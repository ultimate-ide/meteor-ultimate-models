_.extend(Ultimate, {
  ...
  _assignCollection: function(collection, methods) {
	...
	_shouldNotAddHook: function(name) {
		this.hookProto._addedHooks = this.hookProto._addedHooks || {};
		var shouldNotAddHook = this.hookProto._addedHooks[name]; //only add the hook once, the first time
		this.hookProto._addedHooks[name] = true;
