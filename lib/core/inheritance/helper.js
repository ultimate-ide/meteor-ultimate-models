...
InheritanceHelper.extend({
	transformClass: function() {
	  ...
	  initializing = false; //because 'initializing' var is in this closure during 'new this.parent' cuz 'this.Parent === Class' below
		...
		var Class = function Class() {
			...
			else return this.construct.apply(this, arguments); //todo: optimize to not call 2x when descendant of model/form
	  }
	...
	attachPrototype: function(Class) {
		...
		InheritanceHelper.classes[this.className] = Class;
	},
	...
	assignCollection: function(collection, methods) {
		...
		collection._transform = function(doc) {
		  return doc.className ? new InheritanceHelper.classes[doc.classname](doc) : new this.Class(doc);
		}.bind(this);
	},
