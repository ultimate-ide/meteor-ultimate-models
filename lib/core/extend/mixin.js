var instanceRegex = /^construct$|^class$|^className$|^__type$|^parent$|^constructor$|^onStartup$|^___proto$/,
	staticRegex = /^construct$|^className$|^__type$|^onStartup$|^parent$/;
...
Function.extend({
	...
	mixinStatic: function(Parent) { 
		for(var prop in Parent) {
			if(Parent.hasOwnProperty(prop) && !instanceRegex.test(prop)) {
	...
	mixinInstance: function(Parent) { 
		for(var prop in Parent.prototype) {
			if(Parent.prototype.hasOwnProperty(prop) && !staticRegex.test(prop)) {
