Ultimate('UltimatePubSub').extends({
	...
	createSubscribe: function(name, ModelClass, sub) {	
		...
		ModelClass[methodName] = function(options, relations, aggregates, callbacks) {
			var subscriber = this._subsManager || Meteor;
			
			if(!this._shouldCacheSubscription) return subscriber.subscribe(pubSubName, options, relations, aggregates, callbacks);
