Ultimate('UltimateRouter').extends(UltimateFacade, {
	...
	onFacadeStartup: function() {	
		console.log('FACADE START', this.getMethods());
		...
		_.each(routes, function(config, key) {
			if(_.isFunction(config)) {
				...
				if(configString.indexOf('subscribe(') > -1) config = {waitOn: config};
				else if(configString.indexOf('this.render') > -1) config = {action: config};
				else if(configString.indexOf('this.redirect') > -1) config = {action: config};
	...
	_routeController: function(options) {
		...
		return this._controller = this.controller ? GLOBALSCOPE[this.controller].extend(options) : RouteController.extend(options);
	},
	_enableIronRouterControllerInheritance: function(controller, controllerName) {
		GLOBALSCOPE[controllerName] = controller; 
	},
