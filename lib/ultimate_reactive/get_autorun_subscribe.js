UltimateReactive.extend({
	runReactiveMethods: function() {
		console.log('REACTIVE METHODS', this.className, this.ar, this.sub, this.subLimit)
	...
	__combineHandlers: function(handlers) {
		...
		else if(!_.isEmpty(handlers) || _.isFunction(handlers)) allHandlers.push(handlers); //eg: sub: ['self', params] || sub: 'self'
	...
	_runSubscribe: function(subscribeParams, isLimit) {	
		...
		if(_.isArray(subscribeParams)) {
			...
			var subFunc = function(computation, limit) {
				...
				console.log('HELP', subscribeParams);
