	...
	_assignMixinEvents: function() {
		...
		_.each(allEvents, function(eventHandlers, name) {
			this.___proto[name] = function() {
				...
				_.some(eventHandlers, function(handler) {
					if(callNext === false) return true;
					else return callNext = handler.apply(this, args);
				}, this);
				
				return callNext; //returning false will double as the standard way of returning false from an event handler
			};
		});
	},
