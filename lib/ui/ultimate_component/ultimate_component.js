UltimateComponent = Ultimate('UltimateComponent').extends(UltimateReactive, {
	...
	setupCallbacks: function() {
		this.template.onCreated = function() {
			...
			var callNext;
			_.some(this.onCreatedCallbacks, function(cb) {
				if(callNext === false) return true;
				else callNext = cb.call(this);
			}, this);
		}.bind(this);
		...
		this.template.onRendered = function() {
			var callNext;
			_.some(this.onRenderedCallbacks, function(cb) {
				if(callNext === false) return true;
				else callNext = cb.call(this);
			}, this);
		}.bind(this);
		...
		this.template.onDestroyed = function() {
			var callNext;
			_.some(this.onDestroyedCallbacks, function(cb) {
				if(callNext === false) return true;
				else callNext = cb.call(this);
			}, this);
		}.bind(this);
