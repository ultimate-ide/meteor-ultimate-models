UltimateModel = Ultimate('UltimateModel').extends(UltimateForm, {
	...
	persist: function(attVals, cb) {
		...
		var attributes = this.getMongoAttributes(true, null, true, true);
	...
	insert: function(attributes, cb) {
		this._id = this.db().insert(attributes, function(error) {
	...
	update: function(attributes, cb) {
		this.db().update(this._id, {$set: attributes}, function(error) {
			if(error) throw new error;
			else this.refresh(cb);
		}.bind(this));
