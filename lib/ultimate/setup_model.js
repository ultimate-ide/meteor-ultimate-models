_.extend(Ultimate, {
  ...
  _assignCollection: function(collection, methods) {
	...
	_shouldNotAddHook: function(name) {
		this.hookProto.addedHooks = this.hookProto.addedHooks || {};
		var shouldNotAddHook = this.hookProto.addedHooks[name]; //only add the hook once, the first time
		this.hookProto.addedHooks[name] = true;
