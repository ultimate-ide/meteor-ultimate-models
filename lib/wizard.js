...
Template.wizard.created = function() {
  ...
  wizardsById[id] = wizardsById[id] || new Wizard(this);
	wiz.d = this.wizard = wizardsById[id];
};
...
Template.wizard.helpers({
  innerContext: function(outerContext) {
    var wizard = wizardsById[this.id], 
    ...
    var innerContext = {
      ...
      stepsTemplate: this.stepsTemplate || 'wizardSteps',
			backNextTemplate: this.backNextTemplate || 'instance_wizard_back_next'
    };

    _.extend(innerContext, outerContext);
		console.log('INNER CONTEXT', innerContext);
    return innerContext;
  },
  ...
  activeStepTemplate: function(ga) {
		console.log('ACTIVE STEP TEMPLATE', this);
...
var Wizard = function(template) {
  ...
  _.extend(this, _.pick(template.data, ['id', 'route', 'steps', 'clearOnDestroy', 'model', 'onComplete']));

	console.log('GRAND WIZARD', this);
	this.steps = this.steps || this.model.wizards()[this.id];
