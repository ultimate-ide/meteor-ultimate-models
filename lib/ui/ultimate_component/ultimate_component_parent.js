UltimateComponent = Ultimate('UltimateComponentParent').extends(UltimateReactive, {
	...
	onChildClassBeforeStartup: function() {
		if(this.className == 'UltimateComponent' 
			|| this.className == 'UltimateComponentModel'
			|| this.className == 'UltimateDatatableComponent') return;
		
		var UC = this.createNew();
		
		UC.rememberComponent();
		if(UC.copyDatatableTemplate) UC.copyDatatableTemplate(); //let UltimateDatatableComponent copy template first 
