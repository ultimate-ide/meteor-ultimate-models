...
Setup.prototype = {
	...
	mixins: function() {
		...
		_.each(mixins, function(mixin) {
			UltimateClone.deepExtendOwn(this.proto, mixin.prototype);
		}.bind(this));
	},
	...
	onChildClassBeforeStartup: function() {
		//ACTUAL CALL: 
		if(!this.proto.hasOwnProperty('onChildClassBeforeStartup') && !this.proto.hasOwnProperty('abstract')) {  //it's only called from __extend.js if parent class says
			this.proto.onChildClassBeforeStartup.call(this.proto); //run on child classes only
		}
		
		//this is a rare implementation so nested children's and parents' onChildClassBeforeStartup() is all called
		if(this.proto.hasOwnProperty('onChildClassBeforeStartup')) { 
			if(this.proto.parent.onChildClassBeforeStartup) {
				
				//ASSIGNMENT ONLY:
				this.proto.onChildClassBeforeStartup = function() { //make it so the new onChildClassBeforeStartup also calls the old one too
					this.proto.parent.onChildClassBeforeStartup.call(this.proto);
					this.proto.onChildClassBeforeStartup.call(this.proto);
				}.bind(this);	
			}
		}
	},
	...
	onStartup: function() {
		this._onStartup(this.methods.onStartup);
	},
	_onStartup: function(method) {
	...
	onChildClassStartup: function() {
		//ACTUAL CALL: 
		if(!this.proto.hasOwnProperty('onChildClassStartup')) { //it's only called from __extend.js if parent class says
			this.proto.onChildClassStartup.call(this.proto); //run on child classes only
			this._onStartup(this.proto.onChildClassStartup);
		}
		
		//this is a rare implementation so nested children's and parents' onChildClassStartup() is all called
		if(this.proto.hasOwnProperty('onChildClassStartup')) { 
			if(this.proto.parent.onChildClassStartup) {
				...
				this.proto.onChildClassStartup = function() { //make it so the new onChildClassStartup also calls the old one too
					this._onStartup(this.proto.parent.onChildClassStartup);
					this._onStartup(this.proto.onChildClassStartup);
				}.bind(this);	
			}
		}
