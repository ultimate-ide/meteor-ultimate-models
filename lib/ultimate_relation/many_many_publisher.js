Ultimate('UltimateRelationsManyManyPublisher').extends({
	...
	setupRelation: function(rel) {
		...
		//call setupRelation() in else statements because being re-run as part of resetRelationFunc if
		//developer used a function to return his relation and performed dynamic behavior there
		if(!this.throughPublisher) {
			this.throughPublisher = new UltimateRelationsThroughPublisher(this.publisher, throughRelation);		
		}
		else this.throughPublisher.setupRelation(throughRelation); //calling again if this.resetRelationFunc triggers it
