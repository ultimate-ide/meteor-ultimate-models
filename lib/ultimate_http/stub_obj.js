UltimateHTTP.extend({
	generateServerStub: function(meth) {
		...
		var ClassStub = Ultimate.clientClasses[this.className]
		
		if(this.isStatic) ClassStub['staticMethods'].push(meth);
		else ClassStub['instanceMethods'].push(meth);
	},
	generateClassStub: function() {
		var ClassStub = Ultimate.clientClasses[this.className]; //will use already created ClassStub if 2nd+ method
		if(!ClassStub) { 
			ClassStub = Ultimate.clientClasses[this.className] = {}; //allow access by object reference
