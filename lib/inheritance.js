(function(globalScope){
    ...
    // Create a new Class that inherits from this class
  	Function.prototype.extends = function(Parent, methods) {
			...
			//this is some wack ass code, but client side the globalScope var is not the same, probably due to the build process
			Parent = globalScope[Parent.getClassName()]; //the issue is that Parent by itself is the version of it before it's extended
			
			initializing = true; //Instantiate a base class,
			var prototype = new Parent; //but only create the instance,
