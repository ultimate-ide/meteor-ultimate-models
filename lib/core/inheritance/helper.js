...
InheritanceHelper.extend({
	transformClass: function() {
	  ...
	  initializing = false; //because 'initializing' var is in this closure during 'new this.parent' cuz 'this.Parent === Class' below
		...
		var Class = function Class() {
			...
			if(originalConstructor) return originalConstructor.apply(this, arguments); //call the original constructor
			else return;
	  }
	...
	typePrefix: function() {
		if(this.Parent.is('form')) return 'form_';
		else if(this.Parent.is('model')) return 'model_';
		else if(this.Parent.is('template')) return 'template_';
		else return '';
	}
});
...
InheritanceHelper.extendStatic({
	...
	reservedWordsRegex: /^construct$|^class$|^className$|^__type$|^parent$|^constructor$|^___proto$|^createNew$|^onChildClassStartup$/,
		...
		if(_.isObject(collectionName)) collection = collectionName;
			...
			if(_.isArray(collectionName)) {
				...
				_.each(parts, function(part) {
					collectionObjectName += part[0].toUpperCase() + part.slice(1).toLowerCase();
				});	
			}
			
			console.log("COLLECTION NAME", collectionObjectName);
			collection = globalScope[collectionObjectName] = new Meteor.Collection(collectionName);
		}
		
		methods.collection = collection;
	},
	assignCollectionTransform: function(Class) {
		Class.prototype.collection._transform = function(doc) {
		  return new Class(doc);
		};
	},
	
	
	setupTemplate: function(methods, templateName) {
