Ultimate('UltimateComponentParent').extends(UltimateReactive, {
  ...
  _reservedRegex: /^(onCreated|onRendered|onDestroyed|click|dblclick|focus|blur|change|mouseenter|mouseleave|mousedown|mouseup|keydown|keypress|keyup|touchdown|touchmove|touchup)(\s|$)/,
  _eventsRegex: /^(click|dblclick|focus|blur|change|mouseenter|mouseleave|mousedown|mouseup|keydown|keypress|keyup|touchdown|touchmove|touchup)(\s|$)/,
	...
	onBeforeStartup: function() {
		this.rememberComponent();
    this.emit('beforeComponentStartup'); //let UltimateDatatableComponent copy template first
	...
	_resolveHelper: function(method, prop) {
		if(_.isArray(method)) return this._helperShortcut(prop);
			...
			var returnVal = method.apply(this, arguments);
